...
export const useSmartGuidesPlugin = (): CanvasPlugin => {
  ...
  return {
    ...
    onTransformUpdate: (e, ctx) => {
      // CRITICAL: Only snap when MOVING. Scaling/Rotating should not snap to prevent jitter.
        ...
        prev.shapes.forEach(s => {
          ...
          aAnchorsX.forEach(av => bAnchorsX.forEach(bv => {
            ...
            if (diff < minDX) { 
              minDX = diff; 
              snapDX = bv - av; 
              guides.push({ type: 'X', val: bv, target: b });
            }
          }));
          ...
          aAnchorsY.forEach(av => bAnchorsY.forEach(bv => {
            ...
            if (diff < minDY) { 
              minDY = diff; 
              snapDY = bv - av; 
              guides.push({ type: 'Y', val: bv, target: b });
            }
          }));
        });
    ...
    onRenderForeground: (ctx: PluginContext) => {
      ...
      activeGuides.forEach(g => {
        if (g.type === 'X') {
          ...
          c.beginPath();
          c.moveTo(g.val, minY - 10 / zoom);
          c.lineTo(g.val, maxY + 10 / zoom);
          c.stroke();
        } else {
          const minX = Math.min(aAABB.x, g.target.x);
          const maxX = Math.max(aAABB.x + aAABB.w, g.target.x + g.target.w);
          c.beginPath();
          c.moveTo(minX - 10 / zoom, g.val);
          c.lineTo(maxX + 10 / zoom, g.val);
          c.stroke();
        }
      });
