...
export const useTransformPlugin = (): CanvasPlugin => {
    ...
    ctx.setState(prev => {
      ...
      if (transformType === 'ROTATE') {
        ...
        if (isShiftPressed.current && ['tl', 'tr', 'bl', 'br'].includes(activeHandle)) {
          ...
          scaleX = maxScale * Math.sign(scaleX);
          scaleY = maxScale * Math.sign(scaleY);
        }
      }
        ...
        return list.map(s => {
          ...
          if (transformType === 'MOVE') {
            ...
            if (s.type === 'line' && activeHandle?.startsWith('p')) {
              ...
              const pMoving = { x: worldX, y: worldY };
              ...
              news.rotation = activeHandle === 'p1' ? Math.atan2(pFixed.y - pMoving.y, pFixed.x - pMoving.x) : newRotation;
              news.x = newCenter.x - newLength / 2; 
              news.y = newCenter.y - snap.height / 2;
            } else {
              const uiShape = UIShape.create(snap);
              const absSX = Math.abs(scaleX), absSY = Math.abs(scaleY);
              const updates = uiShape.transform({ width: snap.width * absSX, height: snap.height * absSY, scaleX: absSX, scaleY: absSY });
              const finalW = updates.width ?? (snap.width * absSX);
              const finalH = updates.height ?? (snap.height * absSY);
              const vOldLocal = toLocalVector((snap.x + snap.width/2) - fixedPoint.x, (snap.y + snap.height/2) - fixedPoint.y, baseRotation);
              const eSX = snap.width !== 0 ? (finalW / snap.width) * Math.sign(scaleX) : 1;
              const eSY = snap.height !== 0 ? (finalH / snap.height) * Math.sign(scaleY) : 1;
  ...
  const findHandleAt = (ctx: PluginContext, x: number, y: number) => {
    ...
    const rotPosWorld = isSingle ? { x: cx + (0)*cos - (-sh/2 - p - 30/zoom)*sin, y: cy + (0)*sin + (-sh/2 - p - 30/zoom)*cos } : { x: cx, y: (rect!.y) - p - 30 / zoom };
  ...
  return {
    ...
    onMouseDown: (e, hit, ctx) => {
      ...
      if (handleData.h?.startsWith('p')) {
        const { p1, p2 } = getLinePoints(mainShape!);
        const fixed = handleData.h === 'p1' ? p2 : p1;
      ...
      const h = handleData.h as ResizeHandle;
      const rect = isSingle && mainShape ? null : getMultiAABB(shapes, selectedIds);
      const p = VISUAL_PADDING / ctx.state.zoom, rotation = (isSingle && mainShape) ? mainShape.rotation : 0;
      const cos = Math.cos(rotation), sin = Math.sin(rotation);
      const cx = isSingle && mainShape ? mainShape.x + mainShape.width / 2 : (rect!.x + rect!.w / 2);
      const cy = isSingle && mainShape ? mainShape.y + mainShape.height / 2 : (rect!.y + rect!.h / 2);
      const sw = isSingle && mainShape ? mainShape.width : rect!.w, sh = isSingle && mainShape ? mainShape.height : rect!.h;
    ...
    onRenderForeground: (ctx) => {
      ...
      c.save(); c.setTransform(1, 0, 0, 1, 0, 0); c.translate(offset.x, offset.y); c.scale(z, z);
      ...
      if (isSingle && mainShape) {
        c.save(); const cx = mainShape.x + mainShape.width / 2, cy = mainShape.y + mainShape.height / 2;
