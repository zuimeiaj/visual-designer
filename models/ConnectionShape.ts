...
export const PORT_OFFSET = 24; // 强制垂直段的长度
...
export class ConnectionShape extends UIShape {
  ...
  public drawWithScene(ctx: CanvasRenderingContext2D, scene: Scene, state: CanvasState) {
    ...
    const obstacles = scene.getShapes().filter(s => s.type !== 'connection').map(s => s.getAABB());
    const innerPath = this.calculateSmartPath(pStartOffset, pEndOffset, obstacles, zoom);
    const points = this.simplifyPath([pStartEdge, ...innerPath, pEndEdge]);
    ...
    if (isSelected) {
      ...
      ctx.lineWidth = (this.strokeWidth || 2) + 8 / zoom;
      ctx.stroke();
    }
  ...
  private calculateSmartPath(start: Point, end: Point, obstacles: Rect[], zoom: number): Point[] {
    ...
    obstacles.forEach(obs => {
      xCoords.add(obs.x - margin); xCoords.add(obs.x + obs.w + margin);
      yCoords.add(obs.y - margin); yCoords.add(obs.y + obs.h + margin);
    });
    const sortedX = Array.from(xCoords).sort((a, b) => a - b);
    const sortedY = Array.from(yCoords).sort((a, b) => a - b);

    type Direction = 'h' | 'v' | null;
    const queue: { point: Point; path: Point[]; cost: number; dir: Direction }[] = [{ point: start, path: [start], cost: 0, dir: null }];
    const visited = new Map<string, number>();
    let iterations = 0;
    while (queue.length > 0 && iterations++ < 800) {
      queue.sort((a, b) => (a.cost + Math.abs(a.point.x - end.x) + Math.abs(a.point.y - end.y)) - (b.cost + Math.abs(b.point.x - end.x) + Math.abs(b.point.y - end.y)));
      ...
      const xIdx = sortedX.indexOf(point.x), yIdx = sortedY.indexOf(point.y);
      ...
      for (const next of neighbors) {
        if (!this.isSegmentBlocked(point, next.p, obstacles)) {
          const moveDist = Math.abs(next.p.x - point.x) + Math.abs(next.p.y - point.y);
          const nextCost = cost + moveDist + (dir !== null && dir !== next.d ? 2000 : 0);
  ...
  private isSegmentBlocked(p1: Point, p2: Point, obstacles: Rect[]) {
    ...
    return obstacles.some(obs => {
      if (Math.abs(p1.x - p2.x) < 0.1) {
        if (p1.x > obs.x + 0.5 && p1.x < obs.x + obs.w - 0.5) return !(y2 <= obs.y + 0.5 || y1 >= obs.y + obs.h - 0.5);
      } else if (Math.abs(p1.y - p2.y) < 0.1) {
        if (p1.y > obs.y + 0.5 && p1.y < obs.y + obs.h - 0.5) return !(x2 <= obs.x + 0.5 || x1 >= obs.x + obs.w - 0.5);
      }
      return false;
    });
  }
  ...
  private drawPathWithCorners(ctx: CanvasRenderingContext2D, points: Point[], zoom: number) {
    ...
    for (let i = 1; i < points.length - 1; i++) {
      ...
      // 这里的 1e-6 是为了防止除以 0 导致 radius 变成 NaN
      const radius = Math.min(defaultRadius, d1 / 2.01, d2 / 2.01);
      if (radius > 0.1) {
  ...
  public static getPathPoints(scene: Scene, conn: Shape, zoom: number) {
    ...
    return inst.simplifyPath([pStartEdge, ...inst.calculateSmartPath(pStartOffset, pEndOffset, obstacles, zoom), pEndEdge]);
  }
