    ...
    <div className="h-14 w-full bg-white border-b border-zinc-200 px-4 flex items-center justify-between z-50 shrink-0">
      ...
      <div className="flex items-center gap-4 flex-1">
        ...
        <div className="flex items-center gap-1 bg-zinc-100/50 p-1 rounded-lg">
          <button 
            onClick={resetZoom}
            className="flex items-center gap-1.5 px-2 py-1.5 hover:bg-white hover:shadow-sm text-zinc-600 rounded-md transition-all group"
      ...
      <div className="flex-none px-4 flex justify-center">
        {selectedShape && (
          <Toolbar 
            ...
            onUpdate={(updates, save) => actions.updateShape(selectedShape.id, updates, save)} 
          />
        )}
      </div>
      ...
      <div className="flex items-center justify-end gap-3 flex-1">
        <button 
          ...
          className="flex items-center gap-1.5 px-3 py-1.5 hover:bg-zinc-100 rounded-lg text-[10px] font-bold text-zinc-500 uppercase transition-all"
        >
        ...
        <div className="relative" ref={dropdownRef}>
          ...
          {isExportOpen && (
            <div className="absolute right-0 mt-2 w-64 bg-white rounded-xl shadow-2xl border border-zinc-100 py-1.5 z-[100] animate-in fade-in slide-in-from-top-2 duration-200">
              <div className="px-3 py-1.5 text-[9px] font-bold text-zinc-400 uppercase tracking-widest">{t('app.export')}</div>
...
const DropdownItem: React.FC<{ icon: React.ReactNode, label: string, onClick: () => void, disabled?: boolean }> = ({ icon, label, onClick, disabled }) => (
  <button 
    onClick={onClick} 
    disabled={disabled}
    className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium transition-colors text-left ${disabled ? 'opacity-30 cursor-not-allowed text-zinc-400' : 'text-zinc-600 hover:bg-zinc-50 hover:text-indigo-600'}`}
  >
